...
In addition to that there may be two or three other texts for a
unit.  The file named productions-summary describes the syntax
and semantics of ACT-R productions as described in that unit
and the previous units.  If there is a file named "unit#_code"
that document will describe the ACT-R commands introduced in
the main unit text and also describe the code and ACT-R commands
needed to implement the experiments used for the models in that
unit.  If there is a file named "unit#_modeling" then that will
provide additional information about using the ACT-R software
and Environment tools by walking through the process of fixing
a provided model which does not work correctly.

There are also Jupyter notebooks for running all of the models 
and tasks in the tutorial.  The tutorial.ipynb in this directory
should be used as the starting point to access all of the 
individual unit notebooks to ensure that all of the files are 
accessible from the links included in them.
