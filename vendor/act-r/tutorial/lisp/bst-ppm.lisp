...
; value from the indicated production. The command output
; is temporarily disabled with no-output to avoid the 
; spp command also printing the information.

(defun production-u-value (prod)
  (no-output (caar (spp-fct (list prod :u)))))
...
(load (resolve-path "act-r" "tutorial/unit8/bst-ppm-model.lisp"))
