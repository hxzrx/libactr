...
(load (resolve-path "act-r" "tutorial/unit5/1hit-blackjack-model.lisp"))
...
(defun show-model-cards (mcards ocard)
  (if (buffer-read 'goal)
      (mod-focus-fct (list 'mc1 (first mcards) 'mc2 (second mcards) 'mc3 nil 'mtot nil 
                           'mstart (score-cards mcards) 'mresult nil 'oc1 ocard 
                           'oc2 nil 'oc3 nil 'otot nil 'ostart (score-cards (list ocard)) 
                           'oresult nil 'state 'start))
    (goal-focus-fct (list 'isa 'game-state 'mc1 (first mcards) 
                          'mc2 (second mcards) 'mstart (score-cards mcards)
...
(defun show-model-results (mcards ocards mres ores)
  (if (buffer-read 'goal)
      (mod-focus-fct (list 'mc1 (first mcards) 'mc2 (second mcards) 
...
; onehit-learning requires one parameter which is how many
; 100 hand games to play.  There are two optional parameters
...
; play-against-model requires one parameter which is how many
