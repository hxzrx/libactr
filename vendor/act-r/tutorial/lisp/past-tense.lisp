...
(load (resolve-path "act-r" "tutorial/unit7/past-tense-model.lisp"))
...
(defun make-one-goal ()
  (let ((word (random-word)))
    ...
    (goal-focus (isa goal state start))
    word))
...
(defun add-past-tense-to-memory ()
  (let ((word (random-word)))
    (set-buffer-chunk 'imaginal (list 'verb (first word) 'stem (second word) 'suffix (third word)))
