...
;;; 2025.04.28 Dan [6.2]
;;;             : * Releases the names for the new variables it creates when
;;;             :   the production isn't new along with the names that aren't
;;;             :   used when trying to find new names.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;; General Docs:
;;; 
...
(defun compose-productions (module p1 p1-s p2 p2-s)
  ...
  (let* ((p1-basic-vars (basic-variables module p1))
    ...
    (aif (intersection p1-basic-vars p2-basic-vars)
         (progn
           (multiple-value-setq (p2-s new-bindings new-variables) 
        ...
        (let ((post-conditions nil)
          ...
          (let* ((procedural (get-module procedural))
            ...
            (let* ((new-prod (p-fct (reverse new-p)))
              ...
              (when p3
                (update-params-for-compiled-production p3 new-prod p1 p2 module procedural new-variables)))))))))


(defun update-params-for-compiled-production (p3 new-prod p1 p2 module procedural new-variables)
  (let ((exists (check-for-duplicate-productions module p3))
    ...
    (cond ((null exists)
          ...
          ((null (production-user-created exists))
           ...
           (mapcar 'release-name-fct new-variables)
...
(defun rename-variables (p vars not-vars)
  (do* ((x vars (cdr x))
        (new-var (do ((new-var (new-name-fct (car x))
                   ...
                   (release-name-fct new-var))
                 (do ((new-var (new-name-fct (car x))
                               (new-name-fct (car x))))
                     ((not (find new-var not-vars)) new-var)
                   (release-name-fct new-var))
                 )
        (used-vars (list new-var) (push new-var used-vars))
        (bindings (list (cons (car x) new-var))
                  (push (cons (car x) new-var) bindings)))
       ((null (cdr x))
        (values (replace-variables p bindings) bindings used-vars))))
