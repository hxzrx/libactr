...
;;; 
;;; Bugs        : [ ] If a chunk in DM is explicitly merged with another chunk
;;;             :     that is not it DM it can affect the declarative module's
;;;             :     parameters of the DM chunk which could cause problems,
;;;             :     like in-dm being set to nil.
;;;
...
;;;             :     the penalty value.
;;;             : [ ] Consider using the current goal or imaginal chunk as a
;;;             :     finst marker for context sensitive recently-retrieved.
;;; 
;;; ----- History -----
;;;
;;; 2004.10.15 Dan
...
;;; 2026.02.23 Dan
;;;             : * Minor changes to handling the sim-hook and cache to avoid
;;;             :   some extra tests and locking.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;; General Docs:
;;; 
...
(defun chunks-similarity (dm chunk1 chunk2 sim-hook cache-sim-hook act ms md &optional (trace nil))
  (let ((sim (if sim-hook
                 (if cache-sim-hook
                     (bt:with-lock-held ((dm-param-lock dm))
                       (multiple-value-bind (value exists) (gethash (cons chunk1 chunk2) (dm-sim-hook-cache dm))
                         (if exists 
                             value
                           (let ((result (dispatch-apply sim-hook chunk1 chunk2)))
                             (setf (gethash (cons chunk1 chunk2) (dm-sim-hook-cache dm)) result)))))
