...
;;;             :   chunk already exists and makes sense).  When the aural
;;;             :   request is made then the event is marked as requested and
;;;             :   then will be marked as t if successful (if there's a failure
;;;             :   it means the event left the audicon so there's no need to
;;;             :   mark it as nil).  This prevents the default stuffing of 
;;;             :   attended nil (and explicit aural-location requests) from
;;;             :   getting the chunk that corresponds to the item that has 
;;;             :   already had an attention shift requested for it.
;;; 2025.02.27 Dan [9]
;;;             : * Added the command new-ongoing-sound which is similar to the
;;;             :   new-other-sound command except that it does not have a 
;;;             :   specified duration.  The sound generated with new-ongoing-
;;;             :   sound will exist until the end-ongoing-sound command is 
;;;             :   called with the return value from new-ongoing-sound which
;;;             :   created it.
;;;             : * Cleaned up some unit issues because things aren't always
;;;             :   consistent with respect to how they're specified and how
;;;             :   they're recorded in the chunks.
;;;             : * Added the parameter :audio-ms-times which if true specifies
;;;             :   that the values in the chunks will be ms otherwise they will
;;;             :   all be in seconds.  The default value is nil.
;;;             : * To go along with that the :time-in-ms parameter for all the
;;;             :   new-*-sound commands applies to durations as well as onset
;;;             :   and offset times.
;;;             : * The audicon will print with the units specified by the new
;;;             :   parameter since those will match what's in the chunks.
;;; 2025.05.12 Dan
;;;             : * The module wasn't specifying last-command as a valid query.
;;; 2026.06.19 Dan [9.1]
;;;             : * Updated print-audicon so that it also shows custom features
;;;             :   that are added through new-other-sound.  Also changed it so
;;;             :   print-audicon uses printed-audicon instead of duplicating
;;;             :   the code, and removed print-audicon-feature since it's not
;;;             :   documented or used outside of the module code.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(defclass audio-module (attn-module)
  ((audicon :accessor audicon :initarg :audicon :initform nil)
   ...
   ;; just store them in an alist and use a number as the id
   ;; because I don't expect there to be a lot at any one time
   ;; and a number is easy for recording and passing/using remotely
      
   (ongoing-sounds :accessor ongoing-sounds :initform nil) 
   (ongoing-sound-index :accessor ongoing-sound-index :initform 0)
   (model-definition-over :accessor model-definition-over :initform nil)
   (ms-time-units :accessor ms-time-units :initform nil)
...
(defmethod finished-p ((evt sound-event))
  (when (and (numberp (offset evt)) (>= (mp-time-ms) (offset evt))) t))
...
(defmethod new-sound-event :around ((evt sound-event))
  (let ((evt (call-next-method)))
    (when evt
      ...
      (if (duration evt)
          t
        (let ((module (get-module :audio))) ;; has to exist if it gets to this point
          (bt:with-lock-held ((audicon-lock module))
            (let ((index (incf (ongoing-sound-index module))))
              (push (cons index evt) (ongoing-sounds module))
              index)))))))
          ...
          (t
           (new-sound-event (make-instance 'tone-sound-evt 
                              :onset (cond ((null onset) (mp-time-ms))
                                           ...
                                           (t onset)) 
                              :duration (if time-in-ms duration (safe-seconds->ms duration 'new-tone-sound))
                              :content freq))))))
          ...
          (t
           ...
           (new-sound-event (make-instance 'sound-event 
                              :onset (cond ((null onset) (mp-time-ms))
                                           ...
                                           (t onset))
                              :duration (if time-in-ms duration (safe-seconds->ms duration 'new-other-sound))
                              :content content
                              :delay (if time-in-ms delay (safe-seconds->ms delay 'new-other-sound))
                              :recode (if time-in-ms recode (safe-seconds->ms recode 'new-other-sound))
                              :location location
                              :kind kind
...
(add-act-r-command "new-other-sound" 'external-new-other-sound "Create a new custom sound event for the audio module. Params: 'content' duration delay recode {onset-time {'location' {'kind' {time-in-milliseconds {'extra-features'*}}}}}.")


(defun new-ongoing-sound (content delay recode 
                        &optional onset (location 'external) (kind 'speech) time-in-ms &rest other-features)
  (verify-current-model 
   "No current model found.  Cannot create a new sound."
   (setf other-features (decode-string-names other-features))
   (cond ((not (numberp delay))
          (print-warning "Delay must be a number.  No new sound created."))
         ((not (numberp recode))
          (print-warning "Recode must be a number.  No new sound created."))
         ((not (or (null onset) (numberp onset)))
...
(defun external-new-ongoing-sound (content delay recode 
                                         &optional onset (location 'external) (kind 'speech) time-in-ms &rest other-features)
  (apply 'new-ongoing-sound (decode-string content) delay recode onset (decode-string location) (decode-string kind) time-in-ms (decode-string-names other-features)))


(add-act-r-command "new-ongoing-sound" 'external-new-ongoing-sound "Create a new custom sound event for the audio module which continues until stopped with end-ongoing-sound. Params: 'content' delay recode {onset-time {'location' {'kind' {time-in-milliseconds {'extra-features'*}}}}}.")


(defun end-ongoing-sound (id)
  (verify-current-model 
    "No current model found.  Cannot end an ongoing sound."
    (aif (get-module :audio)
         (bt:with-lock-held ((audicon-lock it))
           (let* ((evt-cons (assoc id (ongoing-sounds it)))
                  (evt (cdr evt-cons))
                  (ms (bt:with-recursive-lock-held ((param-lock it)) (ms-time-units it))))
             (if evt
                 (progn
                   (setf (ongoing-sounds it) (remove evt-cons (ongoing-sounds it)))
                   (setf (duration evt) (- (mp-time-ms) (onset evt)))
                   (setf (offset evt) (mp-time-ms))
                   (schedule-event-now 'audio-event-ended :module :audio :priority 9 :output 'high :params (list evt ms)
                                       :precondition 'aural-location-update-check :maintenance t 
                                       :details (format nil "~s ~s" 'audio-event-ended (ename evt)))
                   t)
               (print-warning "~s is not a valid id for and ongoing sound." id))))
         
         (print-warning "No Audio module found.  Cannot end an ongoing sound."))))

(add-act-r-command "end-ongoing-sound" 'end-ongoing-sound "Stop a sound that was created with new-ongoing-sound. Params: id.")
...
(add-act-r-command "new-word-sound" 'external-new-word-sound "Create a new word sound event for the audio module. Params: word {onset-time {'location' {time-in-milliseconds}}}.")


(defun update-sound-evt (evt ms)
  (mod-chunk-fct (ename evt)
                 (append `(onset ,(if ms (onset evt) (ms->seconds (onset evt)))
                           location ,(location evt)
                           kind ,(kind evt)
                           id ,(ename evt)
                           pitch ,(pitch evt))
                         (when (finished-p evt)
                           `(offset ,(if ms (offset evt) (ms->seconds (offset evt)))
                             duration ,(if ms (duration evt) (ms->seconds (duration evt)))))
                         (mapcan (lambda (x)
...
(defmethod find-sound ((aud-mod audio-module) spec &key (stuffed nil))
  ...
  (bt:with-recursive-lock-held ((param-lock aud-mod)) 
    (let* ((slots (chunk-spec-slot-spec spec))
           ...
           (ms (ms-time-units aud-mod))
           (hook (aural-loc-hook aud-mod)))
      ...
      (let ((matching-chunks (find-matching-chunks (slot-specs-to-chunk-spec slots)
                                                   :chunks (mapcar (lambda (x) (update-sound-evt x ms)) event-ls)
                                                   :variable-char #\&)))
            ...
            (dolist (y matching-chunks)
              (let ((cur-val (fast-chunk-slot-value-fct y slot)))
                (unless (numberp cur-val)
                  (unless (or (eq slot 'offset) (eq slot 'duration)) ;; ignore sounds that haven't stopped yet instead of just failing
                    (setf value :fail)
              ...
              (let* ((chunk (aif forced it (random-item matching-chunks)))
                ...
                (unless (and (chunk-slot-value-fct chunk 'offset) (chunk-slot-value-fct chunk 'duration))
                  (when (offset event)
                    ...
                    (schedule-event (offset event) 'audio-event-ended :module :audio :priority 9 :output 'high :params (list event ms)
...
(defun aural-location-update-check (evt ms)
  (declare (ignore ms))
  (let ((buffer-chunk (buffer-read 'aural-location)))
    (and buffer-chunk (eq (chunk-slot-value-fct buffer-chunk 'id) (ename evt)))))

(defun audio-event-ended (evt ms)
  (update-sound-evt evt ms)
  (schedule-mod-buffer-chunk 'aural-location (list 'offset (if ms (offset evt) (ms->seconds (offset evt)))
                                                   'duration (if ms (duration evt) (ms->seconds (duration evt))))
         ...
         (let ((s-event (find (chunk-slot-value-fct event 'id) (current-audicon aud-mod)
           ...
           (when s-event
             (bt:with-lock-held ((audicon-lock aud-mod))
               (setf (attended-p s-event) 'requested)))
    ...
    (bt:with-lock-held ((audicon-lock aud-mod))
      (setf (audicon aud-mod)
        (dolist (e (audicon aud-mod) new-audicon)
          (if (and (offset e) (< (+ (offset e) (decay-time aud-mod)) (mp-time-ms)))
              (progn
...
(defun reset-audio-module (instance)
  ...
  (bt:with-lock-held ((audicon-lock instance))
    ...
    (setf (ongoing-sounds instance) nil)
    (setf (ongoing-sound-index instance) 0))
  ...
  (setf (model-definition-over instance) nil)
  
  (buffer-requires-copies 'aural-location))


(defun audio-module-model-defined (instance)
  (setf (model-definition-over instance) t))

(defun query-audio-module (aud-mod buffer slot value)
  (bt:with-recursive-lock-held ((param-lock aud-mod))
    (case buffer
      (aural
       (if (member slot '(preparation execution processor modality last-command))
  ...
  (bt:with-recursive-lock-held ((param-lock aud-mod))
    (if (consp param)
        (case (first param)
          ...
          (:audio-ms-times
           (when (model-definition-over aud-mod)
             (print-warning "Changing the :audio-ms-times parameter outside of the model definition could result in inconsistencies."))
           (setf (ms-time-units aud-mod) (cdr param))))
      (case param
        (:digit-detect-delay
        ...
        (:audio-ms-times
           (ms-time-units aud-mod))))))

(defun aural-location-status ()
  (concatenate 'string 
    (format nil "  attended nil          : ~S~%" (query-buffer 'aural-location '(attended  nil)))
    (format nil "  attended requested    : ~S~%" (query-buffer 'aural-location '(attended  requested)))
...
(define-module-fct :audio 
  ...
  (list 
   ...
   (define-parameter :audio-ms-times
     :valid-test 'tornil
     :default-value nil
     :warning "T or nil"
     :documentation "Whether the units for onset, offset, and duration in the audio-event chunks are in milliseconds or seconds")
  ...
  :reset '(reset-audio-module nil audio-module-model-defined)
...
(defun print-audicon ()
  (command-output "~a" (printed-audicon))
  nil)
  ...
  (let ((module (get-module :audio)))
    (if module
        (let ((s (make-string-output-stream))
              (ms (bt:with-recursive-lock-held ((param-lock module)) (ms-time-units module)))
              (location-len 12)
              (kind-len 13)
              (content-len 16)
              (other-features nil)
          ...
          (dolist (x (current-audicon module))
            (let ((l-len (length (format nil "~s" (location x))))
                  (k-len (length (format nil "~s" (kind x))))
                  (c-len (length (format nil "~s" (content x)))))
              (when (> l-len location-len)
                (setf location-len l-len))
              (when (> k-len kind-len)
                (setf kind-len k-len))
              (when (> c-len content-len)
                (setf content-len c-len))
              
              (dolist (y (other-features x))
                (let ((l (length (format nil "~s" (third y)))))
                  
                  (aif (assoc (second y) other-features)
                       (when (> l (cdr it))
                         (setf (cdr it) l))
                       (push (cons (second y) (max l (length (format nil "~s" (second y))))) other-features))))))
              
          (format s "Sound event     Att  Detectable  ~v,a  ~v,a  ~v,a  Onset     Offset    Sound ID" 
            kind-len "Kind" content-len "Content" location-len "Location")
          (dolist (o other-features)
            (format s "  ~v,a" (cdr o) (car o)))
          (format s "~%")
          
          (format s "--------------  ---  ----------  ~v,,,'-a  ~v,,,'-a  ~v,,,'-a  --------  --------  --------"
            kind-len "" content-len "" location-len "")
          ...
          (dolist (x (sort (copy-list (current-audicon module)) #'< :key 'onset))
            ...
            (format s "~16a~5A~12A~v,A  ~v,s  ~v,s  ~:[~8f~;~8d~]  ~:[~2* ongoing~;~:[~8f~;~8d~]~]  ~8a"
              (ename x)
              (if (eq (attended-p x) 'requested)
                  "REQ"
                (attended-p x))
              (detectable-p x)
              kind-len
              ...
              (location x)
              ms
              (if ms (onset x) (ms->seconds (onset x)))
              (offset x)
              ms
              (when (offset x) (if ms (offset x) (ms->seconds (offset x))))
              (sname x))
            
            
            (dolist (o other-features)
              (format s "  ~v,a" (cdr o)
                (aif (find (car o) (other-features x) :key 'second)
                     (third it)
                     "")))
