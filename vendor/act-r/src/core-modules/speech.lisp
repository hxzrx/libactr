...
;;; 2025.02.28 Dan
;;;             : * register-articulation-time now has an optional parameter
;;;             :   which if t means the time specified is in ms instead of
;;;             :   seconds.
;;; 2026.03.03 Dan [7.2]
;;;             : * Added the :hear-self parameter which can be used to avoid
;;;             :   sending sounds to audio when not needed for efficiency.
;;;             :   Possible values are t (default) send both speak and 
;;;             :   subvocalize, speak only send speak, subvocalize only send
;;;             :   subvocalize, nil send neither.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(defclass speech-module (pm-module)
  ((syllable-rate :accessor s-rate :initform 150)
   ...
   (hear-self :accessor hear-self :initform t))
  (:default-initargs
    :version-string "7.2"
    :name :SPEECH))


(defmethod register-art-time ((spch-mod speech-module) (text string) (time number) time-in-ms)
  (bt:with-recursive-lock-held ((param-lock spch-mod))
    (setf (gethash text (art-time-ht spch-mod)) (if time-in-ms
                                                    time
                                                  (safe-seconds->ms time 'register-articulation-time)))))
...
(defmethod queue-output-events ((spch-mod speech-module) (mvmt speak))
  (bt:with-recursive-lock-held ((param-lock spch-mod))
    (let ((hear-self (hear-self spch-mod)))
      (when (or (eq hear-self t) (eq hear-self 'speak))
...
(defmethod queue-output-events ((spch-mod speech-module) (mvmt subvocalize))
  (bt:with-recursive-lock-held ((param-lock spch-mod))
    (let ((hear-self (hear-self spch-mod)))
      (when (or (eq hear-self t) (eq hear-self 'subvocalize))
    ...
    (if (consp param)
        (case (car param)
          ...
          (:subvocalize-detect-delay
           ...
           (cdr param))
          (:hear-self
           (setf (hear-self speech) (cdr param))))
      (case param
        (:syllable-rate
...
(define-module-fct :speech 
  ...
  (list 
   ...
   (define-parameter :hear-self
     :valid-test 'test-hear-self 
     :default-value t
     :warning "t, nil, speak, or subvocalize"
     :documentation "Send speech module output to the audio module."))
...
(defun register-articulation-time (string time &optional time-in-ms)
   ...
   (verify-current-model 
    ...
    (aif (get-module :speech)
         (cond ((or (not (numberp time)) (minusp time))
               ...
               (t
                (register-art-time it string time time-in-ms)))
...
(add-act-r-command "register-articulation-time" 'register-articulation-time "Set the time required to speak a string of text in seconds or milliseconds. Params: string time {milliseconds?}.")
