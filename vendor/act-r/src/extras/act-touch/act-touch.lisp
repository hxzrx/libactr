...
;;; 2024.09.12 Dan Bothell
;;; * Fixed start-hand-at-touch to use bt:with-recursive-lock instead of just
;;;   with-lock since the lock is a recursive one.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  (verify-current-model 
   ...
   (let ((installed-touch (find "touch" (current-devices "motor") :test 'string-equal :key 'second)))
     ...
     (let ((module (get-module :touch-interface)))
       (if module
           (if (valid-hand hand)
               (let ((pos (bt:with-recursive-lock-held ((touch-module-lock module)) (touch-module-position module))))
