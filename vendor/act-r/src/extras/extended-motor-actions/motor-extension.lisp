...
;;; 2024.09.12 Dan
;;;             : * Fixed a with-lock-held of a recursive-lock to 
;;;             :   with-recursive-lock-held instead in release-all-fingers.
;;; 2024.11.21 Dan
;;;             : * To make it work better with other motor extensions check the
;;;             :   type of the action before accessing some of the accessors
;;;             :   for the dual-hand-movement-style class.
;;;             : * Also have the custom prepare-movement catch any movement-
;;;             :   style and assume that it has a hand so it can set the
;;;             :   appropriate processor stage.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;; General Docs:
;;; 
    ...
    (bt:with-recursive-lock-held ((param-lock module))
      (when proc 
        (if (listp proc)
          ...
          ;; assume both when a single value is given
          ;; warn if the dual stages is enabled

          (progn
            (when (two-proc module)
              (print-warning "Dual motor processor stages enabled but no hand specified for processor state change.~%Both hands will be set."))

            (setf (first (proc-s module)) proc)
            (setf (second (proc-s module)) proc))))
      ...
      (when exec 
        (if (listp exec)
          ...
          (progn
            (when (two-exec module)
              (print-warning "Dual motor execution stages enabled but no hand specified for execution state change.~%Both hands will be set."))

            (setf (first (exec-s module)) exec)
            (setf (second (exec-s module)) exec)))))
...
;; catch non-dual-hand-movement-style actions too and if an action doesn't specify a hand
;; consider it as both

(defmethod prepare-movement ((module dual-execution-motor-module) (mvmt movement-style))
  (let ((hand (if (slot-exists-p mvmt 'hand)
                  (hand mvmt)
                (progn
                  (print-warning "~s motor action does not specifiy a hand so the dual motor module extension is assuming both hands."
                                 (style-name mvmt))
                  'both))))
  
  (bt:with-recursive-lock-held ((state-lock module))
    (change-state module :prep 'BUSY :proc (list hand 'BUSY))
    ...
    (schedule-event-relative (seconds->ms (fprep-time mvmt)) 'preparation-complete :time-in-ms t 
                             ...
                             :details (concatenate 'string (symbol-name 'preparation-complete) " " (princ-to-string (request-time mvmt))
                                        " style " (symbol-name (type-of mvmt)) " hand " (symbol-name hand)))
    ...
    (unless (relay-hand-requests module) ;; if the buffers sent them to manual don't send them back
      (when (or (eq hand 'left) (eq hand 'both))
      ...
      (when (or (eq hand 'right) (eq hand 'both))
        (schedule-event-now 'module-request 
                            :module (my-name module)
    ...
    (bt:with-recursive-lock-held ((param-lock module))
      (let ((extension (extension module))
        ...
        (let ((hands (if (eq (hand mvmt) 'both) (list 'left 'right) (list (hand mvmt))))
          ...
          (when (and (subtypep (type-of mvmt) 'dual-hand-movement-style) (two-exec-p mvmt))
          ...
          (when (and (subtypep (type-of mvmt) 'dual-hand-movement-style) (release-if-down mvmt))
          ...
          (bt:with-recursive-lock-held ((hand-tracker-lock extension))
            (when (and (subtypep (type-of mvmt) 'dual-hand-movement-style) (finger-based-style mvmt))
              (dolist (hand hands)
                (dolist (finger fingers)
...
(defmethod release-all-fingers ((mtr-mod dual-execution-motor-module) request-spec)
  (if (eq (bt:with-recursive-lock-held ((state-lock mtr-mod)) (prep-s mtr-mod)) 'busy)
