...
;;; 2024.09.17 Dan
;;;             : * Test for single-threaded to exclude the json package 
;;;             :   modifications since the real json package won't be
;;;             :   available under the single-threaded mode and a dummy that
;;;             :   doesn't encode anything will be in its place.
;;; 2024.09.18 Dan
;;;             : * Load-act-r-code in single-threaded mode will swap the 
;;;             :   dummy packages into place before loading the file and then
;;;             :   restore things when done (like require-extra).
;;;             : * Instead of relying on usocket to load split-sequence just
;;;             :   create a replacement instead of loading the quicklisp 
;;;             :   library.
;;; 2026.02.03 Dan [6.0]
;;;             : * For possible speed purposes added the option of using named
;;;             :   pipes instead of the socket.  It requires connecting via the
;;;             :   socket first, and then connecting the pipes as described
;;;             :   in the remote doc and below.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;; General Docs:
;;; 
...
;;; close: Takes no parameters. Returns true.
;;;        Safety call that a client should make when it is going to disconnnect
;;;        so the dispatcher can remove it cleanly at that point instead of 
;;;        waiting for the closed connection to get detected. The downside to
;;;        not calling it is that if there is activity during relevant to the
;;;        closed connection (for example a command monitored by the client) 
;;;        it could result in error messages with the closed client being
;;;        detected that way, but the dispatcher will still close it cleanly at
;;;        that point. 
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;; Public API:
...
;;; 
;;; Switching from a TCP/IP connection to named pipes.
;;;  Pipes are typically faster than TCP/IP connections, and can be used after
;;;  first connecting via the socket on the same machine.  It will require two
;;;  pipes, one for sending communications "to" ACT-R and one that receives the
;;;  communications "from" ACT-R.  
;;;
;;;  This is the process to switch to pipes:
;;; 
;;;  - Call the start-pipe-connection command passing it a string which will be
;;;    used as the id for connecting (which needs to be unique at that time).
;;;    If it returns true, then continue with the pipe connection, but if not 
;;;    then either the provided id is not unique or pipes are not available.
;;;  - Monitor the "pipe-to-actr-result" and "pipe-from-actr-result" commands.
;;;    They will be passed an id and the result for a pipe connection.
;;;  - Create a pipe (either the to or from).
;;;  - Call the "connect-pipe-X-actr" (X being to or from) command passing the
;;;    id and path of the pipe.  If it returns true, then open the pipe and wait
;;;    for a result with your id from the "pipe-X-actr-result" command being 
;;;    monitored.
;;;  - If that result is true, then connect the other pipe direction if not
;;;    already connected.
;;;  - If both connections are successful then call the "finish-pipe-connection"
;;;    command passing the id.  If that returns true then the pipes are ready
;;;    to be used.
;;;  - The close notification should then be sent to the TCP/IP connection.
;;;
;;;  The pipes connection requires one minor change to the communication.  In
;;;  addition to the 0x04 character to end a message it there must also be a
;;;  newline after the 0x04 because the of possibility for line buffering on
...
(defstruct (piped-handler (:include handler))
  in out (in-lock (bt:make-lock)) (out-lock (bt:make-lock)))
...
#-:single-threaded (defmethod json::encode-json ((s symbol) &optional (stream json::*json-output*))
...
#-:single-threaded (setf json::*lisp-identifier-name-to-json* 'identity)
  ...
  (handler-case
      (let ((items (append (mapcar (lambda  (x)
                                     (list (bt:with-lock-held ((handler-command-lock x))
                                             (handler-name x))
                                           (if (piped-handler-p x) 
                                                 (format nil "Pipe connection")
...
(defun return-result (handler p success result)
  ...
  (when (pending-request-id p)
          (let (res json-result)
            ...
            (handler-case
                (if (piped-handler-p handler)
                    (bt:with-lock-held ((piped-handler-out-lock handler))
                      (format (piped-handler-out handler) "~a~%" json-result) ;; add a newline because of potential blocking issues
                      (force-output (piped-handler-out handler)))
...
(defun split-host-name-string (delimiter string)
  (do* ((str string (subseq str (1+ pos)))
        (pos (position delimiter string) (position delimiter str))
...
(defun start-des (&optional (create t) (given-host *default-host*) (remote-port 2650))
  ...
  (let* ((starting-port remote-port)
         (host (if given-host
                   (progn
                     (setf *server-host* (ignore-errors (map 'vector 'parse-integer (split-host-name-string #\. given-host))))
                     given-host)
...
;;; Code for switching from the socekt to pipes with lots of handshaking
;;; and allowing for the possiblity of concurrent attempts from different
;;; sources.

(defvar *pipe-connect-table* (make-hash-table :test 'equalp))

(defstruct pipe-connect in out)

(defun can-use-pipes ()
  ;; Should have some real tests, like OS and whether the current connection
  ;; is on the same machine, but for now just assume true and let the other
  ;; pieces catch the problems and stop things.
  t)

(defun start-pipe-connection (name)
    (if (can-use-pipes)
       (let ((exists (gethash name *pipe-connect-table*)))
         (if exists
            (print-warning "attempt to switch to pipe with id ~s has already been started." name)
           (prog1 
               t 
             (setf (gethash name *pipe-connect-table*) (make-pipe-connect)))))
       (print-warning "pipe connection is not available.")))


(defun connect-pipe-in (name pipe-path)
  (let ((exists (gethash name *pipe-connect-table*)))
    (if (and exists (null (pipe-connect-in exists)))
        (if (probe-file pipe-path)
            (prog1 
                t 
              (bt:make-thread (lambda ()
                                (handler-case 
                                    (let ((in-stream (open pipe-path :direction :input)))
                                      (if in-stream                       
                                          (progn
                                            (setf (pipe-connect-in exists) in-stream)
                                            (dispatch-apply "pipe-to-actr-result" name t))
                                        (dispatch-apply "pipe-to-actr-result" name (print-warning "Could not open pipe ~s to ACT-R for id ~s" pipe-path name))))
                                  (error (x) 
                                    (dispatch-apply "pipe-to-actr-result" name (print-warning "Error ~/print-error-message/ encountered while connecting pipe to ACT-R for id ~s." x name)))))))
          
          (dispatch-apply "pipe-to-actr-result" name (print-warning "pipe ~s as the to ACT-R connection for id ~s could not be found." pipe-path name)))
      
      (dispatch-apply "pipe-to-actr-result" name (print-warning "pipe to ACT-R connection for id ~s ~:[has not been started~;already attempted~]." name exists)))))


 
(defun connect-pipe-out (name pipe-path)
  (let ((exists (gethash name *pipe-connect-table*))) 
    (if (and exists (null (pipe-connect-out exists)))
        (if (probe-file pipe-path)
            (prog1 
                t
              (bt:make-thread (lambda ()
                                (handler-case 
                                    (let ((out-stream (open pipe-path :direction :output :if-exists :overwrite)))
                                      (if out-stream                       
                                          (progn
                                            (setf (pipe-connect-out exists) out-stream)
                                            (dispatch-apply "pipe-from-actr-result" name t))
                                        (dispatch-apply "pipe-from-actr-result" name (print-warning "Could not open pipe ~s from ACT-R for id ~s" pipe-path name))))
                                  (error (x) 
                                    (dispatch-apply "pipe-from-actr-result" name (print-warning "Error ~/print-error-message/ encountered while connecting pipe from ACT-R for id ~s." x name)))))))
          
          (dispatch-apply "pipe-from-actr-result" name (print-warning "pipe ~s as the from ACT-R connection for id ~s could not be found." pipe-path name)))
      
      (dispatch-apply "pipe-from-actr-result" name (print-warning "pipe from ACT-R connection for id ~s ~:[has not been started~;already attempted~]." name exists)))))



(defun finish-pipe-connection (name)
  (let ((exists (gethash name *pipe-connect-table*)))
    (cond ((null exists)
           (print-warning "Cannot finish connection for id ~s because it has not been started." name))
          ((null (pipe-connect-in exists))
           (print-warning "Cannot finish connection for id ~s because it has not connected a to ACT-R pipe." name))
          ((null (pipe-connect-out exists))
           (print-warning "Cannot finish connection for id ~s because it has not connected a from ACT-R pipe." name))
          (t 
           (prog1
               t
             (let* ((in-stream (pipe-connect-in exists))
                    (out-stream (pipe-connect-out exists))
                    (handler (make-piped-handler :in in-stream :out out-stream :socket "Piped-connection")))
               (setf (handler-thread handler) (bt:make-thread (lambda ()
                                                                (let ((*package* *default-package*))
                                                                  (unwind-protect
                                                                      (process-external-input handler in-stream)
                                                                    ...
                                                                    (handler-case 
                                                                        (progn 
                                                                          ...
                                                                          (close in-stream)
                                                                          (close out-stream))
                                                                      (error (x) (send-error-output "Error ~/print-error-message/ encountered while cleaning up a terminated piped connection." x))))))
                                                             :name "process-external-piped-input"))
               (bt:with-lock-held ((dispatcher-handler-lock *dispatcher*))
                 (push handler (dispatcher-connections *dispatcher*)))))))))

;;;; end pipe connection code
                         ...
                         (let ((method (assoc :method message :test 'string=))
                           ...
                           (cond (method
                                  ...
                                  (let* ((m (cdr method))
                                    ...
                                    (unless (or (and p i) (and (null (cdr p)) (null i)))
                                           ...
                                           (action (cond 
                                                    ...
                                                    ((and (string-equal m "set-name")
                                                     ...
                                                     (make-action :type 'set-name
                                                                  :parameters params
                                                                  :evaluator handler))
                                                    ((and (string-equal m "close")
                                                          (= (length params) 0)
                                                          (null id))
                                                     ;; It's a notification which has no id
                                                     ;; and gets no response, and ends the connection.
                                                     (return-from process-external-input nil)))))
...
(defmethod send-remote-command ((handler handler) action command parameters)
  (let (p)
    (bt:with-lock-held ((handler-sent-requests-lock handler))
...
(defmethod send-remote-command ((handler piped-handler) action command parameters)
  (let (p)
    (bt:with-lock-held ((handler-sent-requests-lock handler))
      (setf p (make-pending-request :action action :id (incf (handler-id handler))))
      (push p (handler-sent-requests handler)))
    ...
    (bt:with-lock-held ((pending-request-lock p))
      ...
      (handler-case 
          (progn
            (bt:with-lock-held ((piped-handler-out-lock handler))
              (format (piped-handler-out handler) "{\"method\": \"evaluate\", \"params\": ~a, \"id\": ~d}~c~%" 
                (json:encode-json-to-string (cons (dispatch-command-underlying-function command) (cons (model-to-name (action-model action)) parameters))) (pending-request-id p) (code-char 4))
              (force-output (piped-handler-out handler)))
            (loop
              (bt:condition-wait (pending-request-cv p) (pending-request-lock p))
...
;;; Add the command that will allow a connected client switch to using a pipe instead

(add-act-r-command "switch-to-pipe" 'switch-to-pipe "Use the provided named pipes for communication instead of the socket. Params: pipe-to-actr pipe-from-actr")
...
;;; add the commands for switching to the piped connection

(add-act-r-command "start-pipe-connection" 'start-pipe-connection "Attempt to switch a connection to using pipes returns true if pipe connection available. Params: id.")
(add-act-r-command "connect-pipe-to-actr" 'connect-pipe-in "After start-pipe-connection call this.  Params: id path_of_pipe.")
(add-act-r-command "connect-pipe-from-actr" 'connect-pipe-out "After start-pipe-connection call this. Params: id path_of_pipe.")
(add-act-r-command "pipe-to-actr-result" nil "Signal from ACT-R indicating success of the connect-pipe-to-actr attempt.  Params: id result")
(add-act-r-command "pipe-from-actr-result" nil "Signal from ACT-R indicating success of the connect-pipe-from-actr attempt.  Params: id result")
(add-act-r-command "finish-pipe-connection" 'finish-pipe-connection "After connecting the to and from pipes call this to complete the process. Params: id.")
