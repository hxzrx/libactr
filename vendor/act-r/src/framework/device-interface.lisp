...
;;; 2025.02.14 Dan 
;;;             : * notify-device now prints a when there's an error in the
;;;             :   interface function called instead of just passing the nil
;;;             :   and error message back to the caller.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
         ...
         (let ((d (bt:with-lock-held ((device-lock di)) (device di))))
           (if (find device-list d :test 'equalp)
               (let ((device (gethash (second device-list) (bt:with-lock-held ((device-tables-lock di)) (device-table di)))))
                 (dispatch-apply (act-r-device-notification device) device-list features))
