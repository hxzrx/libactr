...
;;;; Suite :libactr/server (defined in tests/test-server.lisp). Drives the routing
...
(defpackage :libactr/subtraction-adapter-test
  (:use :cl :5am))
(in-package :libactr/subtraction-adapter-test)
...
session, so each problem must end before the next starts)."
  (let ((sid (libactr/server:server-start-session s student problem "sub")))
    (%step s sid ones)
    (%step s sid tens)
...
SUBTRACT-TENS-DIRECT, done."
  (let ((s (%server)))
    (unwind-protect
         (let ((sid (libactr/server:server-start-session s "s1" "52-18" "sub")))
           (let ((r1 (%step s sid 4)))
             ...
             (is (null (libactr:step-done?
                        (libactr/subtraction-adapter:make-subtraction-adapter) r1 nil))))
           (let ((r2 (%step s sid 3)))
             (multiple-value-bind (status name) (%status-name r2)
               (is (eq :on-path status))
               (is (string= "SUBTRACT-TENS-DIRECT" name)))
             (is (libactr:step-done?
                  (libactr/subtraction-adapter:make-subtraction-adapter) r2 nil))))
...
intent); tens 2 -> on-path SUBTRACT-TENS-DIRECT, done."
  (let ((s (%server)))
    (unwind-protect
         (let ((sid (libactr/server:server-start-session s "s2" "47-25" "sub")))
  ...
  (let ((s (%server)))
    (unwind-protect
         (progn
           ...
           (let ((m (libactr/server:server-student-mastery s "s3")))
             (is (= 2 (length m)))
             (is (= 2 (getf (%entry m :borrow) :total)))
             (is (= 1 (getf (%entry m :column-subtract) :total))))
           (%full-problem s "s4" "47-25" 2 2)
           (let ((m (libactr/server:server-student-mastery s "s4")))
             (is (= 1 (length m)))
...
BUGGY-BORROW-IGNORE with feedback present."
  (let ((s (%server)))
    (unwind-protect
         (let ((sid (libactr/server:server-start-session s "s5" "43-27" "sub")))
...
off-path-buggy BUGGY-ALWAYS-BORROW."
  (let ((s (%server)))
    (unwind-protect
         (let ((sid (libactr/server:server-start-session s "s6" "46-23" "sub")))
...
subtraction fact slipped)."
  (let ((s (%server)))
    (unwind-protect
         (let ((sid (libactr/server:server-start-session s "s7" "43-27" "sub")))
           (multiple-value-bind (status name) (%status-name (%step s sid 5))
             (is (eq :off-path-buggy status))
             (is (string= "BUGGY-OFF-BY-ONE" name)))
           (let ((m (progn (libactr/server:server-end-session s sid)
                           (libactr/server:server-student-mastery s "s7"))))
...
(test subtraction-adapter.unclassified-off-path
  ...
  (let ((s (%server)))
    (unwind-protect
         (let ((sid (libactr/server:server-start-session s "s8" "43-27" "sub")))
           (is (eq :off-path (libactr:trace-result-status (%step s sid 9)))))
      (libactr/server:stop-tutor-server s))))
...
problems stay out of the bug-path corpus."
  (let ((s (%server)))
    (unwind-protect
         (let ((sid (libactr/server:server-start-session s "s9" "52-17" "sub")))
           (multiple-value-bind (status name) (%status-name (%step s sid 5))
             (is (eq :on-path status))
             (is (string= "SUBTRACT-ONES-BORROW" name)))
           (is (eq :off-path (libactr:trace-result-status (%step s sid 4)))))
      (libactr/server:stop-tutor-server s))))
    ...
    (unwind-protect
         (progn
           (signals libactr:bad-tutor-request
             (libactr/server:server-start-session s "sx" "5-18" "sub"))
           (signals libactr:bad-tutor-request
             (libactr/server:server-start-session s "sx2" "52-52" "sub"))
           (multiple-value-bind (r status)
               (libactr/server::handle-start s `(("student_id" . "sx")
                                             ("problem_id" . "5-18")
                                             ("model_id" . "sub")))
             (is (= 400 status))
             (is (search "positive answer" (getf r :error))))
           (let ((sid (libactr/server:server-start-session s "sy" "52-18" "sub")))
             (signals libactr:bad-tutor-request
               (libactr/server:server-step-session
             ...
             (%step s sid 4)
             (%step s sid 3)
             (signals libactr:bad-tutor-request (%step s sid 9))))
      (libactr/server:stop-tutor-server s))))
...
(test subtraction-adapter.out-of-order-400
  ...
  (let ((s (%server)))
    (unwind-protect
         (let ((sid (libactr/server:server-start-session s "oo" "52-18" "sub")))
           (signals libactr:bad-tutor-request
             (libactr/server:server-step-session s sid '(("type" . "digit")))) ; no value
           (libactr/server:server-step-session s sid '(("type" . "digit") ("value" . "4"))) ; ones
           (libactr/server:server-step-session s sid '(("type" . "digit") ("value" . "3"))) ; tens -> done
           (signals libactr:bad-tutor-request
             (libactr/server:server-step-session
