...
;;;; A CONSUMER of the libactr core (independent system). Demonstrates the interface
...
(defpackage :libactr/addition-tutor
  (:use :cl)
  (:nicknames :addition-tutor)
  (:export #:make-tutor #:tutor-step #:demonstrate
           #:load-tutor-model #:dm-next #:chunk-slot-val))
(in-package :libactr/addition-tutor)

;; --- buggy library (libactr-authored, each with feedback) -------------------------
;;
  ...
  (list
   (libactr:make-production
    ...
    (list (libactr:make-buffer-pattern
           'goal := 'add
           (list (libactr:make-slot-test 'sum :literal nil))))
    (list (libactr:make-action := 'goal '((count . one))))
    nil :buggy "Count on from zero — your sum already holds the first number.")
   (libactr:make-production
    'buggy-wrong-next
    (list (libactr:make-buffer-pattern
           'goal := 'add
           (list (libactr:make-slot-test 'sum :variable '=sum))))
    ;; RHS expresses a non-adjacent next total; matches a student who jumps.
    (list (libactr:make-action := 'goal '((sum . skipped))))
...
when demonstrate is invoked from an arbitrary caller package (e.g. CL-USER)."
  (let ((*package* (find-package :libactr/addition-tutor)))
    (let ((md (libactr:compile-model
      ...
      (setf (libactr:model-definition-productions md)
            (append (libactr:model-definition-productions md) (buggy-rules)))
      md)))
...
;; must NOT be prefixed `libactr:` — it is not a core symbol.
(defun chunk-slot-val (chunk slot)
  (cdr (assoc slot (libactr:chunk-slots chunk))))
...
retrieval buffer) that trace-step traces against."
  (let ((chunks (libactr:model-definition-chunks md)))
...
(defun make-tutor (arg1 arg2)
  ...
  (let* ((md (load-tutor-model))
         (session (libactr:start-session md 'student (list arg1 arg2))))
    ;; override the initial-goal template with the problem-specific goal
    (setf (libactr:buffer-chunk (libactr:session-state session) 'goal)
          (libactr:make-chunk :isa 'add :slots `((arg1 . ,arg1) (arg2 . ,arg2) (sum . nil))))
...
(defun prime-num (tutor val)
  ...
  (let ((md (libactr:session-model (tutor-session tutor)))
        (state (libactr:session-state (tutor-session tutor))))
...
(defun trace-and-advance (tutor intent)
  ...
  (libactr:step-session (tutor-session tutor) intent))
...
Primes retrieval=number(count), traces count -> (dm-next count), advances."
  (let* ((goal (libactr:buffer-chunk (libactr:session-state (tutor-session tutor)) 'goal))
         (count (chunk-slot-val goal 'count))
         (newcount (dm-next (libactr:session-model (tutor-session tutor)) count)))
    (prime-num tutor count)
    (trace-and-advance tutor (libactr:make-step-intent :assignments `((goal count ,newcount))))))

(defun render-step (action result)
  "Print one traced step's diagnosis."
  (format t "~&[~A] ~A via ~A~@[ — ~A~]  (KC ~A)~%"
          action
          (ecase (libactr:trace-result-status result)
            (:on-path "on-path")
            (:off-path-buggy "off-path (buggy)")
            (:off-path "off-path (unclassified)"))
          (or (and (libactr:trace-result-production result)
                   (libactr:production-name (libactr:trace-result-production result)))
              "—")
          (libactr:trace-result-feedback result)
          (or (and (libactr:trace-result-events result)
                   (libactr:kc-event-kc (first (libactr:trace-result-events result))))
              "—")))
...
the student-visible trace-result (initialize / increment-sum / terminate step)."
  (ecase action
    ...
    (:next-total
     ...
     (prime-num tutor (chunk-slot-val (libactr:buffer-chunk (libactr:session-state (tutor-session tutor)) 'goal) 'sum))
     (let ((r (trace-and-advance tutor intent)))
       (when (eq (libactr:trace-result-status r) :on-path)
         (advance-count! tutor))
       r))
    (:submit
...
(defun intent/start ()           (libactr:make-step-intent :assignments '((goal sum five) (goal count zero))))
(defun intent/next-total (n)     (libactr:make-step-intent :assignments `((goal sum ,n))))
(defun intent/submit (answer)    (libactr:make-step-intent :assignments `((goal count nil) (goal sum ,answer))))
(defun intent/buggy-count-one () (libactr:make-step-intent :assignments '((goal count one))))
