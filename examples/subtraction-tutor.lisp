...
;;;; :libactr/subtraction-tutor. Buggy rules are appended (libactr-only). KC attribution
...
;;;; in this file.
(defpackage :libactr/subtraction-tutor
  (:use :cl)
  (:nicknames :subtraction-tutor)
  (:export #:load-subtraction-model #:bug-specs))
(in-package :libactr/subtraction-tutor)
...
that IS correct. Symbols land in :libactr/subtraction-tutor (this file's
...
buggy library. Model symbols land in :libactr/subtraction-tutor (*PACKAGE*
...
direct columns share :column-subtract."
  (let ((*package* (find-package :libactr/subtraction-tutor)))
    (let ((md (libactr:compile-model
      ...
      (setf (libactr:model-definition-productions md)
            (append (libactr:model-definition-productions md)
                    (mapcar #'libactr:bug-production (%validate-specs! (bug-specs)))))
      md)))
